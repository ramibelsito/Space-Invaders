//Si funciona esto ya tenemos todo el camino completo para testear en el thread.
//Después habría que generalizar más casos de colisión
...
/*Mira todas las balas, una por una. Si están activas, chequea si están en colisión con otro objero.
...
int main (void)	//Main de testeo
{
	...
	Lo mismo con la bala, ya no le voy a decir cuál bala es, le voy a mandar todo el array y
	la función va a tener que encontrarla.*/

	i=n; j=n-1;	//Uso los últimos para ver si recorrió todo el array
	...
	balas[i].posicion[0]=400;	balas[i].posicion[1]=1600;	//Uso la última bala para ver si mira todas
	...
	check_bala(aliens, barreras, &jugador, balas);											//Debería colisionar
	...
	check_bala(aliens, barreras, &jugador, balas);											//No debería colisionar
	...
	/*No debe colisionar con más de un objeto a la vez,
	así que voy a crear DOS aliens en colisión con la bala.
	Lo que tiene que hacer el programa es colisionar con
	uno solo de ellos. Veamosss*/

	balas[i].colisiones = 0;aliens[j].colisiones = 0;aliens[j-9].colisiones=0;	//Limpio el campo de colisión

	aliens[j-9].posicion[0]=500;aliens[j-9].posicion[1]=1500;//Debería ya detectar este colisionando y no al próximo
	aliens[j-9].hitbox[0]=100;	aliens[j-9].hitbox[1]=300;

	aliens[j].posicion[0]=500;	aliens[j].posicion[1]=1500;	//Uso el último alien (ya debería haber colisionado con el otro)
	aliens[j].hitbox[0]=100;	aliens[j].hitbox[1]=300;
	
	balas[i].posicion[0]=400;	balas[i].posicion[1]=1600;	//Uso la última bala para ver si mira todas
	balas[i].hitbox[0]=200;		balas[i].hitbox[1]=50;

	balas[i].dibujo=0x1111;	aliens[j].dibujo=0xafd;	aliens[j-9].dibujo=0xddd;	//Les asigno un "dibujo" (porque están activos)

	check_bala(aliens, barreras, &jugador, balas);												//Debería colisionar con j-9
	printf("%f  %f  %p\n", balas[i].posicion[0], balas[i].posicion[1], balas[i].dibujo);		//Debería estar desactivado
	printf("%f  %f  %p\n", aliens[j].posicion[0], aliens[j].posicion[1], aliens[j].dibujo);		//No debería estar desactivado
	printf("%f  %f  %p\n", aliens[j-9].posicion[0], aliens[j-9].posicion[1], aliens[j-9].dibujo);//No debería estar desactivado
	
	//Chequeo que esté cargada la colisión contra el j-9
	if ( (balas[i].colisiones == OBJETO(ALIEN,j-9)) && (aliens[j-9].colisiones == OBJETO(PROYECTIL,i)) )
	{
		printf("Todo 0k. Diushtax wazouski 8 mil porciento tres estrellas el que no salta es holandés, francés, inglés, lo que se te cante\n");
	}	//Ayuda :(

	//Chequeo que siga limpio el campo de colisión (porque el j no colisionó con nada)
	printf("%d\n",aliens[j].colisiones);

	return 0;
}	//Llegó la hora de la verdad...		-	 FUNCIONA TODO SUUUUUUUU	-
